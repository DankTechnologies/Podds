<script lang="ts">
	...
	async function playEpisode(feed: Feed, episode: Episode) {
		if (!feedExists) {
			await feedService.addFeedAndEpisodes(feed, episodes);
		}
	...
	async function downloadEpisode(feed: Feed, episode: Episode) {
		if (!feedExists) {
			await feedService.addFeedAndEpisodes(feed, episodes);
		}
...
<div class="share-container">
	...
	{:else if feed}
		...
		{:else}
			...
			{/if}
			<EpisodeList {episodes} {activeEpisodes} isShare={true} hideImages />
		{/if}
	{/if}
</div>
